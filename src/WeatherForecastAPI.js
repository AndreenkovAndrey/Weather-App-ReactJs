...
export const WeatherForecastAPI = (city) => {
  const [
    ...
    setСurrentWeatherAndForecastResponse,
  ] = useState(null);
  const FORECAST_DAYS = 3;

  useEffect(() => {
    if (city) {
      getСurrentWeatherAndForecastResponse(city);
    }
  }, [city]);

  async function getСurrentWeatherAndForecastResponse(city) {
    ...
    try {
      ...
      if (response.status === 200 && response.data) {
        setСurrentWeatherAndForecastResponse(response.data);
      }
    } catch (error) {
      console.error(error);
    }
